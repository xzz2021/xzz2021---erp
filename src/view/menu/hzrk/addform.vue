...
<el-dialog title="花纸录入" :visible.sync="show" width="700px"  center>
    <Form ref="hzrk" :model="hzrk" v-show="true" :rules="ruleValidate" :label-width="80">
      <Row>
        <FormItem label="供货商" prop="company" >
            <Input v-model="hzrk.company" placeholder="供货商" clearable></Input>
        </FormItem>
        <FormItem label="订单号" prop="order" >
            <Input v-model="hzrk.order" placeholder="订单号" clearable></Input>
        </FormItem>
        <FormItem label="送货日期" prop="date" >
           <Input v-model="hzrk.date"
           placeholder="送货日期" clearable></Input>
        </FormItem>
        <FormItem label="版号" prop="model">
            <Input v-model="hzrk.model" placeholder="版号" clearable></Input>
        </FormItem>
        <FormItem label="花色" prop="color">
            <Select v-model="hzrk.color" placeholder="花色" filterable multiple allow-create @on-create="handleCreatepro">
              <Option v-for="color in colorList" :value="color.value" :key="color.value">{{ color.label }}</Option>
            </Select>
        </FormItem>
        <FormItem label="花纸名称" prop="modelname">
            <Select v-model="hzrk.modelname" placeholder="花纸名称"  filterable multiple allow-create @on-create="handleCreatepro">
              <Option v-for="modelname in proList" :value="modelname.value" :key="modelname.value">{{ modelname.label }}</Option>
            </Select>
        </FormItem>
         <FormItem label="实收数" prop="num" >
            <Input v-model="hzrk.num" placeholder="实收数" clearable></Input>
        </FormItem>

        <FormItem label="规格" prop="spec" >
            <Input v-model="hzrk.spec" placeholder="规格" clearable></Input>
        </FormItem>
        <FormItem label="色数" prop="conum" >
        <Input v-model="hzrk.conum" placeholder="色数" clearable></Input>
        </FormItem>
        <FormItem label="备注" prop="remark" width="200px">
            <Input v-model="hzrk.remark" type="textarea" placeholder=""></Input>
        </FormItem>
        </Row>
        </Form>
         <span slot="footer" class="dialog-footer">
            <el-button @click="handleReset('hzrk')">清空输入</el-button>
            <el-button type="primary" @click="handleSubmit('hzrk')">提交</el-button>
        </span>
    </el-dialog>
</template>
<script>
import { subhzrk } from '../../../api/forms'
export default {
  data () {
    return {
      show: false,
  ...
  methods: {
    ...
    getToday () {
      ...
      this.hzrk.date = m + '月' + d + '日'
    },
    handleSubmit (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          this.hzrk.modelname = (this.hzrk.modelname).join('+')
          this.hzrk.color = (this.hzrk.color).join('+')
          subhzrk(this.hzrk).then(res => {
            if (res.status === 1) {
